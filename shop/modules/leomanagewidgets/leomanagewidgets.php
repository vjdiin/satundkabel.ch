...
class LeoManagewidgets extends Module {
    ...
    private function _processHook($hook_name){
        ...
        $submitNewsletter = Tools::isSubmit('submitNewsletter');
        if ($submitNewsletter) {
            $this->_clearCache('group.tpl', $this->name.'|'.$hook_name);
        }
        if($this->_leotype == 1){
            if (!file_exists(_PS_MODULE_DIR_ . 'leotempcp/classes/widgetbase.php')){
                return $this->l("Please install leotemcp module");
        ...
        if ($submitNewsletter || !$this->isCached($tplFile, $this->getCacheId($this->name.'|'.$hook_name)))
        {
